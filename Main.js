...
function setOrthoAndPerspectiveCameras(canvas) {
  g_OrthographicCamera = new THREE.OrthographicCamera(canvas.offsetWidth / -60, canvas.offsetWidth / 60, canvas.offsetHeight / 60, canvas.offsetHeight / -60, 1, 20000);
  g_PerspectiveCamera = new THREE.PerspectiveCamera(70, canvas.offsetWidth / canvas.offsetHeight, 1, 20000);
  ...
  g_OrthoCameraControl = new THREE.OrbitControls(g_OrthographicCamera, g_Renderer.domElement);
  g_OrthoCameraControl.target.set(0, 0, -2)
  g_PerspCameraControl = new THREE.OrbitControls(g_PerspectiveCamera, g_Renderer.domElement);
...
function addSceneOptions() {
  ...
  datGuiFolder.addColor(options, 'backgroundColor').name("Background").onChange(() => {
    g_Scene.background = new THREE.Color(options.backgroundColor);
  });
  datGuiFolder.add(options, 'isPerspective').name("Perspective").onChange(switchBetweenOrthoAndPersp);
}
...
function draw3DObject() {
  ...
  mainObjectUI.add(meshObject.material, 'opacity').min(0.0).max(1).step(0.1).name("Opacity");
...
function getBoundingBoxOfmainObject() {
  ...
  mainObjectBoundingBox.setFromCenterAndSize(bbCenter, new THREE.Vector3(size, size, 0));
...
function addSwappingGridAroundmainObject() {
  ...
  gridUI.add(g_SwappingGrid.userOptions, "showDivisions").name("Show Divisions (ft)").onChange(function () { g_SwappingGrid.redrawTheAxisTriad() });
}
